    ...
    [HttpTool postWithUrl:url params:nil contentType:CONTENTTYPE success:^(id responseObject) {
        if (ResponseObject_RC == 0) {
         ...
         //   headbaseImageString = data[@"backimg"];
            ...
            [headbaseView setImageWithURL:[NSURL URLWithString:data[@"backimg"]] success:^(UIImage* image, BOOL cached) {
                NSLog(@"cached %d", cached);
                succImage = YES;
                xiugaiBaseImage = headbaseView.image;
            }
...
- (void)setPersonButton
{
   ...
   // ament.baseImageStr = headbaseImageString;
    ament.baseImage = xiugaiBaseImage;
...

{
    ...
    if (pickertag == 777) {
        ...
        [HttpTool uploadImageWithUrl:url image:info[UIImagePickerControllerEditedImage] completion:^(id responseObject) {
            ...
            xiugaiBaseImage = info[UIImagePickerControllerEditedImage];
        }
            errorBlock:^(NSError* error){
            }];
    }
