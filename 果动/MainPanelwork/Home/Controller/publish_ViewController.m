...
- (void)viewDidLoad
{
    ...
    amentButton.frame = CGRectMake(Adaptive(330), Adaptive(27), Adaptive(35), Adaptive(24.8));
...
- (void)onCreate
{
    ...
    _textView.frame = CGRectMake(0, 0, viewWidth, Adaptive(150));
    _textView.font = [UIFont fontWithName:FONT size:Adaptive(17)];
    ...
    textnumberLabel = [[UILabel alloc] initWithFrame:CGRectMake(_textView.bounds.size.width - Adaptive(30) - Adaptive(15), _textView.bounds.size.height - Adaptive(12) - Adaptive(5), Adaptive(30), Adaptive(12))];
    textnumberLabel.textColor = [UIColor colorWithRed:1 green:125 / 255.0 blue:40 / 255.0 alpha:1];
    textnumberLabel.font = [UIFont fontWithName:FONT size:Adaptive(12)];
    textnumberLabel.text = [NSString stringWithFormat:@"%d字", TextNumber];
    [_textView addSubview:textnumberLabel];

    UIView* imgview = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(progressView.frame), viewWidth,Adaptive(180))];
    imgview.backgroundColor = [UIColor whiteColor];
    imgview.userInteractionEnabled = YES;
    [self.view addSubview:imgview];

    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(Adaptive(13), Adaptive(10), Adaptive(100), Adaptive(100))];
...
- (void)share:(UIButton*)sender
{
    if ([[_textView.text trimString] isEmptyString]) {
        ...
        NSDictionary* dict = pvtype == 1 ? @{ @"content" : _textView.text } : @{ @"type" : [NSString stringWithFormat:@"%d", pvtype],
...
- (void)textViewDidEndEditing:(UITextView*)textView
{
    ...
    //通过内容和字体大小计算出宽度
    CGSize textSize = [textView.text boundingRectWithSize:CGSizeMake(textView.bounds.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName : [UIFont fontWithName:FONT size:Adaptive(15)] } context:nil].size;
...
- (void)addImageView:(NSInteger)index
{
    ...
    imageView.frame = CGRectMake(Adaptive(90) * index + Adaptive(10),Adaptive(10), Adaptive(80), Adaptive(80));
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    if (picker == _imagePickerController) {
        ...
        imageSize.width = Adaptive(400);
        imageSize.height = Adaptive(400);
        //  对图片大小进行压缩  获取图片的data
