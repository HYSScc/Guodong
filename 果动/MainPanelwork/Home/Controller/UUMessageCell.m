...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame{
    ...
    UIImage *normal;
    if (message.from == UUMessageFromMe) {
        normal = [UIImage imageNamed:@"talk_bubble_one"];
        
        normal = [normal stretchableImageWithLeftCapWidth:normal.size.width/2 topCapHeight:normal.size.height/2+10 ];
    }
    else{
        normal = [UIImage imageNamed:@"talk_bubbleSelf_one"];
        normal = [normal stretchableImageWithLeftCapWidth:normal.size.width/2 topCapHeight:normal.size.height/2+10 ];
    }
