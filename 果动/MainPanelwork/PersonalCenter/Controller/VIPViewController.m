...
- (void)viewDidLoad {
    ...
    for (int a = 0; a < 4; a++) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(viewWidth/8, (viewHeight-viewHeight/6.67*2)+a*(viewHeight/26.68), viewWidth-viewWidth/4, viewHeight/26.68)];
        label.text = array[a];
        label.textColor = [UIColor colorWithRed:228.00/255 green:221.00/255 blue:216.00/255 alpha:1];
        label.font = [UIFont fontWithName:FONT size:viewHeight/41.6875];
        [self.view addSubview:label];
    }
    
    _textUserName=[[UITextField alloc]initWithFrame:CGRectMake((viewWidth-(viewHeight/2.7676))/2, viewHeight/3, viewHeight/2.7676, viewHeight/15.881)];
    _textUserName.borderStyle=UITextBorderStyleNone;
    _textUserName.placeholder=@"请输入No.开头的6位卡号";
    ...
    _textUserName.font = [UIFont fontWithName:FONT size:viewHeight/41.6875];
    ...
    [_textUserName setValue:[UIFont boldSystemFontOfSize:viewHeight/41.6875] forKeyPath:@"_placeholderLabel.font"];
    [self.view addSubview:_textUserName];
    
    _password=[[UITextField alloc]initWithFrame:CGRectMake((viewWidth-(viewHeight/2.7676))/2,CGRectGetMaxY(_textUserName.frame) + viewHeight/66.7 , viewHeight/2.7676, viewHeight/15.881)];
    ...
    _password.font = [UIFont fontWithName:FONT size:viewHeight/41.6875];
    ...
    [_password setValue:[UIFont boldSystemFontOfSize:viewHeight/41.6875] forKeyPath:@"_placeholderLabel.font"];
    [self.view addSubview:_password];
    
    
    
    loginButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    loginButton.frame = CGRectMake((viewWidth-(viewHeight/2.7676))/2, CGRectGetMaxY(_password.frame) + viewHeight/66.7, viewHeight/2.7676, viewHeight/19.057);
    [loginButton setTintColor:[UIColor whiteColor]];
    [loginButton setTitle:@"验证" forState:UIControlStateNormal];//@"Arial-BoldMT"
    loginButton.titleLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:viewHeight/44.467];
