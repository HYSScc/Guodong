...
-(id) initWithView:(UIView *)view withBlur:(BOOL)blur
{
    ...
    {
        _containerView = view;
        _containerView.frame = CGRectMake(0, 0, viewWidth, viewHeight);
        _isShouldBlur = blur;
...
-(void) initBackgroundWithBlur:(BOOL)blur
{
    ...
    else
    {
        ...
        _backgroundStatic.backgroundColor = BASECOLOR;
    }
...
-(void) initDot
{
    ...
    // init center dot
    UIView *centerDot = [[UIView alloc] initWithFrame:CGRectMake(center.x-10, center.y - 100, 20, 20)];
    centerDot.clipsToBounds = YES;
    centerDot.backgroundColor = [UIColor orangeColor];
...
-(void) initLabel
{
  ...
  //  _label.backgroundColor = [UIColor orangeColor];
   ...
   // _label.font = [UIFont fontWithName:@"Neou-Thin" size:36];
    ...
    // set center
    _label.center = CGPointMake(self.center.x, self.center.y  + _label.bounds.size.height / 2);
...
{
    NSLog(@"font  %@",fontTitleLabel);
    if (_fontTitleLabel == fontTitleLabel)
        return;
    NSLog(@"font 11  %@",fontTitleLabel);
