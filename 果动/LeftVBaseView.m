...
- (void)startRequestClassNumber
{
    ...
    [HttpTool postWithUrl:classnumberURL params:nil contentType:CONTENTTYPE success:^(id responseObject) {
        ...
        if (ResponseObject_RC == 0) {
            class_idArray = [NSMutableArray array];
            classnameArray = [NSMutableArray array];
            classnumberArray = [NSMutableArray array];
            class_imageArray = [NSMutableArray array];
            for (NSDictionary* dict in [responseObject objectForKey:@"data"]) {
                ...
                NSURL* imageUrl = [dict objectForKey:@"imgUrl"];
                [class_imageArray addObject:imageUrl];
            }
            [self createLabel];
        }
        else {
...
- (void)createLabel
{
    ...
    for (int a = 0; a < 2; a++) {
        ...
        [image setImageWithURL:class_imageArray[a]];
        UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, viewWidth / 2, Adaptive(20))];
        ...
        UILabel* classtop = [[UILabel alloc] initWithFrame:CGRectMake((classScrollView.frame.size.width / 2) * a, CGRectGetMaxY(label.frame), image.bounds.size.width, Adaptive(10))];
        ...
        classtop.font = [UIFont fontWithName:FONT size:Adaptive(10)];
        [classScrollView addSubview:classtop];

        UIImageView* image1 = (UIImageView*)[classScrollView viewWithTag:(a + 3) * 1000];
        [image1 setImageWithURL:class_imageArray[a + 2]];
        UILabel* label1 = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, viewWidth / 2, Adaptive(20))];
        ...
        label1.font = [UIFont fontWithName:FONT size:Adaptive(18)];
        [classScrollView addSubview:label1];

        UILabel* classbuttom = [[UILabel alloc] initWithFrame:CGRectMake((classScrollView.frame.size.width / 2) * a, CGRectGetMaxY(label1.frame), image.bounds.size.width, Adaptive(10))];
        ...
        classbuttom.font = [UIFont fontWithName:FONT size:Adaptive(10)];
...
- (void)createImage
{
    ...
    for (int a = 0; a < 2; a++) {
        ...
        [image setImage:[UIImage imageNamed:@"shouye_moren"]];
        image.tag = (a + 1) * 1000;
        ...
        image1.tag = (a + 3) * 1000;
        [image1 setImage:[UIImage imageNamed:@"shouye_moren"]];
