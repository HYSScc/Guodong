...
- (void)createUI {
    ...
    for (int a = 0; a < 2; a++) {
        ...
        imageView.tag = (a + 1) * 3;
        imageView.image = [UIImage imageNamed:@"pay_duihao"];
        ...
        if (a == 1) {
            imageView.image = [UIImage imageNamed:@"pay_duihao_red"];
            makeLabel.text = @"下次再说";
...
- (void)buttonClick:(UIButton *)button {
    ...
    switch (button.tag) {
        case 2:
            imageViewLeft.image  = [UIImage imageNamed:@"pay_duihao_red"];
            imageViewRight.image = [UIImage imageNamed:@"pay_duihao"];
