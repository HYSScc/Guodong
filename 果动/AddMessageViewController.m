...
- (void)sureButtonClick:(UIButton *)button {
    ...
    if ([_classOrShip isEqualToString:@"class"]) {
        ...
        if ([addressView.messageLabel.text length] == 0 ||
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"信息填写不完整" delegate:self cancelButtonTitle:nil otherButtonTitles: nil];
            [alert show];
    ...
    if (!photo) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"电话号码输入有误" delegate:self cancelButtonTitle:nil otherButtonTitles: nil];
        [alert show];
        [NSTimer scheduledTimerWithTimeInterval:1.5f
                                         target:self
                                       selector:@selector(timerFireMethod:)
        ...
        if (buttonIndex == 1) {
            ...
            [HttpTool postWithUrl:url params:dict body:nil progress:^(NSProgress * progress) {
                ...
                if ([[[responseObject objectForKey:@"data"] objectForKey:@"order_type"] isEqualToString:@"gd"]) {
                    ...
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:[NSString stringWithFormat:@"您已成功预约:%@  剩余:%@课时",_className,[[responseObject objectForKey:@"data"] objectForKey:@"package_balance"]] delegate:self cancelButtonTitle:nil otherButtonTitles: nil];
                    [alert show];
                    [NSTimer scheduledTimerWithTimeInterval:1.5f
                                                     target:self
                                                   selector:@selector(timerFireMethod:)
