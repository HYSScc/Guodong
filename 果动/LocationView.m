...
- (void)createUI
{
    self.topLocationImageView = [[UIImageView alloc] initWithFrame:CGRectMake(-Adaptive(5), 0, Adaptive(12), Adaptive(18))];
    ...
    self.locationButton.frame = CGRectMake(0, Adaptive(3), Adaptive(50), Adaptive(20));
    self.locationButton.titleLabel.font = [UIFont fontWithName:FONT size:Adaptive(14)];
...
- (void)locationManager:(CLLocationManager*)manager didUpdateLocations:(NSArray*)locations
{
    ...
    [HttpTool postWithUrl:urlString params:locationDict contentType:CONTENTTYPE success:^(id responseObject) {
        if (ResponseObject_RC == 0) {
            ...
            if (!allName) {
                NSLog(@"定位失败");
                home.alertImageView.frame = CGRectMake(0, -Adaptive(50), viewWidth, Adaptive(50));
            ...
            NSDictionary* dict = @{ @"address" : address,
                @"succ" : [NSString stringWithFormat:@"%d", succ] };
            ...
            CGSize userNameSize = [allName sizeWithAttributes:@{ NSFontAttributeName : [UIFont fontWithName:FONT size:Adaptive(22)] }];
