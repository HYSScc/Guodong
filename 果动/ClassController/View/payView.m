               ...
               viewController:(UIViewController *)viewController
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tongzhi:) name:@"changeMoney" object:nil];
...
- (void)tongzhi:(NSNotification *)notification {
    ...
    payMoney    = _money;
    _classTypes = [notification.userInfo objectForKey:@"types"];
...
- (void)createUIWithPayMoney:(NSString *)money classTime:(NSString *)classtime youhuijuan:(NSString *)juan {
    ...
    //微信支付
    UIImageView *weixinImage = [[UIImageView alloc] initWithFrame:CGRectMake(viewWidth / 6, CGRectGetMaxY(lineTwo.frame) + Adaptive(10), Adaptive(47), Adaptive(61))];
    ...
    //银联支付
    UIImageView *ylImage = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(weixinImage.frame) + viewWidth / 10, CGRectGetMaxY(lineTwo.frame) + Adaptive(12), Adaptive(58), Adaptive(59))];
    ...
    //支付宝支付
    UIImageView *zfbImage = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(ylImage.frame) + viewWidth / 10, CGRectGetMaxY(lineTwo.frame) + Adaptive(19), Adaptive(80), Adaptive(52))];
   ...
   // 区分是否使用优惠劵
    ...
    [HttpTool postWithUrl:kUrl params:requestdict contentType:CONTENTTYPE success:^(id responseObject) {
        ...
        dispatch_async(dispatch_get_main_queue(),
                       ^{
                           [Pingpp createPayment:charge viewController:payViewController appURLScheme:kUrlScheme withCompletion:^(NSString* result, PingppError* error) {
